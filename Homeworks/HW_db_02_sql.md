...
(2 rows)
```
Конмада explain позволяет ознакомиться с планом выполнения нашего запроса, еще в выводе присутствует стоимость выполнения запроса или время выполнения, если я правильно понимаю. Счистаю, что этот параметр может помочь оптимизировать запрос для уменьшения времени, тем самым уменьшив нагрузку на БД.
